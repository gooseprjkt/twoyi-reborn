...
public final class RomManager {
    ...
    private static void createLoaderSymlink(Context context) {
        // Create 64-bit loader symlink
        Path loader64Symlink = new File(context.getDataDir(), "loader64").toPath();
        String loader64Path = getLoaderPath(context);
        createSymlink(loader64Symlink, loader64Path);

        // Create 32-bit loader symlink
        Path loader32Symlink = new File(context.getDataDir(), "loader32").toPath();
        String loader32Path = getLoader32Path(context);
        createSymlink(loader32Symlink, loader32Path);
    }

    private static void createSymlink(Path symlinkPath, String targetPath) {
        try {
            Files.deleteIfExists(symlinkPath);
            Files.createSymbolicLink(symlinkPath, Paths.get(targetPath));
        } catch (IOException e) {
            Log.w(TAG, "symlink creation failed: " + e.getMessage());
            // Don't throw exception as this is not critical for functionality
        }
    }

    private static String getLoader32Path(Context context) {
        ApplicationInfo applicationInfo = context.getApplicationInfo();
        return new File(applicationInfo.nativeLibraryDir, "libloader32.so").getAbsolutePath();
    }
